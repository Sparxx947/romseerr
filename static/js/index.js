const I18N={de:{
 nav_discover:'Entdecken',nav_requests:'Anfragen',nav_users:'👤 Benutzer',nav_settings:'Einstellungen',logout:'🚪 Abmelden',
 ...
 nav_discover:'Discover',nav_requests:'Requests',nav_users:'👤 Users',nav_settings:'Settings',logout:'🚪 Sign out',
 ...
 nav_discover:'Découvrir',nav_requests:'Demandes',nav_users:'👤 Utilisateurs',nav_settings:'Paramètres',logout:'🚪 Déconnexion',
 ...
 nav_discover:'Descubrir',nav_requests:'Solicitudes',nav_users:'👤 Usuarios',nav_settings:'Ajustes',logout:'🚪 Salir',
 ...
 nav_discover:'Scopri',nav_requests:'Richieste',nav_users:'👤 Utenti',nav_settings:'Impostazioni',logout:'🚪 Esci',
...
async function loadJobs(){let r=await fetch('/api/jobs');let d=await r.json();let j=document.getElementById('jobs');
 ...
 d.forEach(o=>{let e=document.createElement('div');e.className='job';let L=stlab(o.state);let right;
  ...
  e.innerHTML=`<div><div>${o.title.replace(/</g,'&lt;')}</div><div class=meta style="color:#8b929e;font-size:11px">👤 <b style="color:#b9c0cc">${(o.user||'—').replace(/</g,'&lt;')}</b> · ${o.platform} · ${o.source}${vs}${vl}${dt?' · '+dt:''} · ${o.msg||''}</div></div><div>${right}</div>`;
  j.appendChild(e);});}
