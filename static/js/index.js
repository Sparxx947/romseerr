...
document.addEventListener('keydown',e=>{
 ...
 if(m&&m.style.display==='block'){closeModal();return;}
 // ZULETZT das Suchfeld (#661). Die Reihenfolge ist die ganze Vorsicht hier: stuende das
 // Leeren vorn, naehme ein Escape dem Dialog das Schliessen weg — und wer einen Dialog
 // schliessen will, will nicht seine Suche verlieren.
 let q=document.getElementById('q');
 if(q&&q.value){sucheLeeren(true);}});
...
function applyI18n(){
 ...
 // `aria-label` gehoert uebersetzt wie alles andere: ein Knopf, dessen einziger Name auf
 // Deutsch steht, heisst fuer einen englischen Vorleser weiterhin „Zurueck". (#661)
 document.querySelectorAll('[data-i18n-al]').forEach(e=>e.setAttribute('aria-label',t(e.dataset.i18nAl)));
 // Neu zeichnen statt ueberleben lassen (#732): Der Quellen-Hinweis steht ausserhalb von
 // `data-i18n`, wird von der Schleife oben also nicht geleert — aber er muesste sonst nach
 // einem Sprachwechsel in der alten Sprache stehen bleiben.
 quellHinweis();
 updateFLabel();}
let cur='s';
...
// EINE Quelle fuers Leeren (#662, erweitert in #661). Der Klick auf die Marke, der
// Leeren-Knopf und Escape gehen alle hier durch — eine zweite Fassung fuer den Knopf haette
// diese hier still ueberschrieben, weil gleicher Name gewinnt.
// `fokus` nur fuer den Knopf und Escape: beim Klick auf die Marke will niemand hinterher
// im Suchfeld stehen.
function sucheLeeren(fokus){let q=document.getElementById('q');if(!q||!q.value)return false;
 q.value='';suchKnoepfe();loadDiscover();if(fokus)q.focus();return true;}
...
function libGruppe(g){
 ...
 let zeilen=auf?g.platforms.map(p=>`<button class="job libzeile"
   aria-label="${(p.name||'').replace(/"/g,'')} — ${p.owned.toLocaleString()} ${t('lib_entries')}"
...
// Ein Zeichen aus den Definitionen einbinden (#658). EINE Stelle, nicht je Aufruf ein
// eigenes SVG-Geruest — sonst laufen `viewBox`, `aria-hidden` und Klassen auseinander.
function zeichen(name,klasse){
 return '<svg class="'+(klasse||'navsym')+'" aria-hidden="true" focusable="false" '
      + 'viewBox="0 0 24 24"><use href="#rs-i-'+name+'"/></svg>';}
...
//
// WARUM — gemessen ueber zehn Reihen-Suchen: 6 bis 36 % der Karten waren Wiederholungen
// desselben Spiels, im schlimmsten Fall zehnmal `mario kart`. Das drueckt neun andere
// Spiele von den ersten Seiten und ist damit dieselbe Beschwerde wie #689 auf einem
// anderen Weg. Die Oberflaeche widersprach sich dabei selbst: Der Sammelknopf zaehlte
// `gkey` und bot „Alle anfragen (25)" an, waehrend 47 Karten danebenlagen.
//
// DIE REIHENFOLGE KOMMT UNVERAENDERT VOM SERVER. Hier wird nur der erste Treffer je
// `gkey` behalten und gezaehlt, nicht neu sortiert — eine zweite Sortierlogik im
// Frontend liefe irgendwann auseinander (so ist es der Zeilenbeschriftung in #632
// ergangen). Dass der Vertreter zum Zeichen auf der Karte passt, entscheidet der
// Sortierschluessel in `do_search`, nicht diese Funktion.
//
// `window.LASTRES` bleibt VOLLSTAENDIG: Die Detailansicht baut ihre Fassungsliste daraus
// (`x.gkey===it.gkey`), und die Sammelanfrage entdoppelt selbst.
function gruppiere(d){let wo={},out=[];
 d.forEach(it=>{let k=it.gkey||it.title;
...
function renderCard(it,fassungen){let c=document.createElement('div');c.className='card';
 ...
 // Erst ab zwei sagt die Zahl etwas — „1 Fassung" an jeder Karte waere Rauschen.
 let fass=(fassungen>1)?' · '+t('versions_n').replace('{n}',fassungen):'';
 ...
 let plat=plattformMarke(it.platform_slug,it.lib_slugs);
 c.innerHTML=`<div class=cover style="${cov}"><span class=badge>${plat}</span><span class=src>${src}</span>`+
  ...
  <div class=body><div class=t>${FAVS.has(norm(it.title||''))?'<span class=favmark title="'+t('favourite')+'">♥</span> ':''}${it.title.replace(/</g,'&lt;')}</div><div class=meta>${sz(it.size)}${settag}${gesperrt}${fass}</div><div class=act></div></div>`;
...
// Die Buehne des Aurora-Designs gehoert zum leeren Zustand: Sobald jemand sucht, ist die
// Trefferliste das Thema und eine halbe Bildschirmhoehe Ueberschrift steht nur im Weg.
// `hidden` statt einer Klasse, damit sie auch fuer Vorleser verschwindet. (#629)
function zeigeBuehne(an){let b=document.getElementById('buehne');if(b)b.hidden=!an;}
// Zurueck und Leeren in der Suchzeile (#661).
//
// Der Zurueck-Knopf ERSETZT die Verlaufsverwaltung aus #194 nicht, er bedient sie: dort
// wurde bewusst kein Knopf gebaut, sondern jeder Ansicht eine Adresse gegeben, damit der
// Browser von sich aus das Richtige tut. `EIGENE_SCHRITTE` weiss, wie viele Eintraege die
// App selbst angelegt hat — ohne diese Frage fuehrte ein `history.back()` aus Romseerr
// hinaus, wenn jemand direkt auf einer Such-Adresse gelandet ist.
//
// Beide Knoepfe erscheinen NUR, wenn sie etwas bewirken. Das ist der Befund aus #638: ein
// Knopf, bei dem nichts passiert, ist von einem kaputten nicht zu unterscheiden.
function suchZurueck(){if(EIGENE_SCHRITTE>0)history.back();}
function suchKnoepfe(){
 let q=document.getElementById('q'),c=document.getElementById('tClear'),b=document.getElementById('tBack');
 if(c)c.style.display=(q&&q.value)?'':'none';
 if(b)b.style.display=EIGENE_SCHRITTE>0?'':'none';}
// Sagt IN der Trefferliste, wie viele Treffer der Plattformfilter gerade zurueckhaelt.
//
// WARUM DAS NOETIG IST — gemessen, nicht befuerchtet: Ein Klick auf eine Entdecken-Karte
// setzt den Filter auf deren Plattform (unten, `c.onclick`) und schreibt ihn nach
// localStorage. Dort bleibt er — ueber Suchen, ueber Neuladen, ueber Tage. „Silent Hill
// Homecoming" fand mit haengengebliebenem `snes` noch 4 Treffer statt 14, weil Ergebnisse
// OHNE erkannte Plattform absichtlich jeden Filter passieren. Die Liste sah damit nicht
// gefiltert aus, sondern so, als gaebe es den Titel kaum.
//
// WARUM DER KNOPF OBEN NICHT REICHT: „Plattformen: 1 gewaehlt" sagt, wie viele Plattformen
// gewaehlt sind — nicht, dass GERADE zehn Treffer fehlen. Zwischen beidem liegt der ganze
// Weg, den niemand geht, wenn eine Suche einfach duenn zurueckkommt. Die Zahl gehoert
// deshalb dorthin, wo das Ergebnis steht, mit dem Weg heraus daneben. (#688)
// --- Quellenstand: sagen, wenn ein Ergebnis ein Rueckfall ist (#732) ---
//
// Seit #726 liefert eine ausgefallene Quelle ihren letzten bekannten Stand statt „keine
// Treffer". Das ist richtig — und war unsichtbar: Die Liste sah aus wie eine frische
// Antwort. Der zweite, im Betrieb HAEUFIGERE Fall ist noch stiller: Faellt eine Quelle
// aus, ohne dass etwas gemerkt war, steuert sie gar nichts bei und sagt nichts. Neun Tage
// Protokoll auf dem laufenden Server: 18 Quellenausfaelle, 0 Rueckfaelle.
//
// DER ZUSTAND STEHT MODULWEIT UND NICHT IM DOM, weil `applyI18n` ihn neu zeichnen muss.
// Sonst uebersteht der Hinweis zwar den Sprachwechsel, steht danach aber in der falschen
// Sprache da — und ein Warnhinweis, der die Sprache nicht mitmacht, ist schlimmer als
// keiner: Man liest ihn nicht und traut ihm nicht.
let QUELLSTAND=null;
// Die drei tragen ihren Namen selbst, in jeder Sprache gleich — hier ist nichts zu
// uebersetzen, und eine i18n-Tabelle voller Eigennamen laeuft nur auseinander.
const QUELLNAMEN={archive:'Archive.org',usenet:'Usenet',filehoster:'Filehoster'};
// Alter als Kurzform („4 Min."), nicht ausgeschrieben („4 Minuten"): Die Kurzform hat
// keinen Plural, und „1 Minuten" waere in jeder der fuenf Sprachen falsch. Genau ist die
// Zahl ohnehin nicht gemeint — sie beantwortet „wie alt ungefaehr".
function quellAlter(s){s=Math.max(0,parseInt(s,10)||0);
 if(s<90)return t('src_age_sec').replace('{n}',s);
 let m=Math.round(s/60);if(m<90)return t('src_age_min').replace('{n}',m);
 return t('src_age_hour').replace('{n}',Math.round(m/60));}
function quellHinweis(){let box=document.getElementById('srchint');if(!box)return;
 box.textContent='';box.style.display='none';
 if(!QUELLSTAND)return;
 let zeilen=Object.keys(QUELLSTAND).sort().map(k=>{let z=QUELLSTAND[k]||{},n=QUELLNAMEN[k]||k;
  return z.state==='stale'?t('src_stale').replace('{s}',n).replace('{age}',quellAlter(z.age))
                          :t('src_down').replace('{s}',n);});
 if(!zeilen.length)return;
 // textContent, nicht innerHTML: Die Namen sind zwar unsere eigenen, aber ein Fremdname
 // aus einer kuenftigen Quelle liefe hier sonst ungeprueft in die Seite.
 box.textContent='⚠ '+zeilen.join(' · ');box.style.display='';}
function platHinweis(hint,n){if(!n||!SELP.size)return;
 let s=document.createElement('span');s.className='plathint';
 s.appendChild(document.createTextNode(' · '+t('plat_hidden').replace('{n}',n)+' '));
 let b=document.createElement('button');b.className='plathint-x';b.textContent=t('plat_drop');
 b.onclick=()=>{clearP();search();};
 s.appendChild(b);hint.appendChild(s);}
async function search(){let q=document.getElementById('q').value.trim();if(!q){loadDiscover();return;}
 ...
 let r=await fetch('/api/search?q='+encodeURIComponent(q)+'&platforms='+[...SELP].join(',')+(RAONLY?'&achievements=1':''));
 let versteckt=parseInt(r.headers.get('X-Platform-Hidden')||'0',10)||0;let d=await r.json();
 // Fehlt die Kopfzeile, war alles frisch — der Hinweis verschwindet dann von selbst. (#732)
 try{let s=r.headers.get('X-Source-Status');QUELLSTAND=s?JSON.parse(s):null;}
 catch(e){QUELLSTAND=null;}
 quellHinweis();
 ...
 // waehrend daneben der Sammelknopf Spiele zaehlte („47 Treffer" und „Alle anfragen
 // (25)"). Jetzt zaehlen beide Spiele; der Unterschied sind genau die schon vorhandenen,
 // denn der Knopf laesst die aus. (#691)
...
// DREI KNOEPFE AUF DER DETAILKARTE, EINE KLASSE (#708). Vorher trugen zwei ihre Farben
// und ihren Eckenradius INLINE und einer kam aus dem Stylesheet — in Aurora unterschieden
// sie sich damit in Grund, Schriftfarbe UND Radius. Inline schlaegt jede Designregel,
// weshalb ausgerechnet der Favoriten-Knopf dem Design folgte und die beiden anderen nicht.
//
// ZEICHEN UND TEXT STEHEN IN EIGENEN KNOTEN, und das ist keine Zier: toggleFav und
// addWishlist setzen textContent — das loescht jedes Kind. Ein Zeichen im Knopf waere
// beim ersten Klick weg. Dieselbe Falle wie #337, nur ueber einen anderen Weg.
 ...
 m.innerHTML=`<div class=box><button class=x onclick="closeModal()">×</button>
  <div class=top><div class=mc style="${it.cover?`background-image:url('${it.cover}')`:''}"></div>
   <div><h2>${it.title.replace(/</g,'&lt;')}</h2>
    ...
    <button class=kartenknopf onclick="reportFromDetail()">${zeichen('probleme')}<span>${t('report_issue')}</span></button>
    <button id=wlbtn class=kartenknopf onclick="addWishlist(this)">${zeichen('listen')}<span>${t('add_wishlist')}</span></button>
    <button id=favbtn class="kartenknopf favbtn${FAVS.has(norm(it.title||''))?' on':''}" onclick="toggleFav(window._detit,this)">${zeichen(FAVS.has(norm(it.title||''))?'herz-voll':'herz')}<span>${t('favourite')}</span></button>
...
async function addWishlist(btn){btn.disabled=true;let it=window._detit||{};
 ...
 // Auch hier nur der Text — das Lesezeichen bleibt stehen. (#708)
 let sp=btn.querySelector('span');
 if(sp)sp.textContent=r.ok?t('wl_added'):(r.msg||t('st_error'));
 else btn.textContent=r.ok?t('wl_added'):(r.msg||t('st_error'));}
...
function openWlImport(){let m=document.getElementById('modal');m.style.display='block';
 m.innerHTML=`<div class=box><button class=x onclick="closeModal()">×</button>
  <h2>${zeichen('listen','navsym ueberschrift')}${t('wl_import')}</h2>
...
async function loadDiscover(){let hint=document.getElementById('hint');hint.style.display='';hint.textContent=t('loading_home');
 // Entdecken fragt die Suchquellen nicht — ein Hinweis der letzten Suche waere hier eine
 // Aussage ueber etwas, das gar nicht abgefragt wurde. (#732)
 QUELLSTAND=null;quellHinweis();
 zeigeBuehne(true);
 ...
 // Zahlen fuer die Buehne — dieselbe Quelle wie die Bibliotheksansicht, kein zweiter
 // Bestand. Die Antwort ist {total, vendors:[{platforms:[…]}]}, KEIN flaches Array: die
 // Feldnamen sind an der laufenden Instanz nachgesehen, nicht geraten. Faellt der Abruf
 // aus, bleibt die Zeile leer statt falsch.
 try{let st=await(await fetch('/api/library/platforms')).json();
     let bm=document.getElementById('bmarke');
     if(bm&&st&&st.total){
       let pl=(st.vendors||[]).reduce((a,v)=>a+((v.platforms||[]).length),0);
       bm.textContent=Number(st.total).toLocaleString()+' '+t('lib_entries')+' \u00b7 '+pl+' '+t('platforms');}
 }catch(e){}
...
// Bestaetigungsdialog statt `confirm()` (#641).
//
// WARUM UEBERHAUPT: `confirm()` zeichnet der Browser. Keine Regel erreicht es, es sieht auf
// allen vier Designs fremd aus — und es friert die Seite fuer die Browsertests ein, weshalb
// mehrere zerstoerende Pfade gar keine Abdeckung hatten.
//
// WARUM `<dialog>` UND NICHT EIN EIGENES OVERLAY: Fokusfalle, Escape und die oberste Ebene
// bringt der Browser mit. Selbstgebaute Modale vergessen davon regelmaessig die Haelfte.
//
// WAS DER NATIVE DIALOG RICHTIG MACHT und hier erhalten bleibt: Der Fokus springt hinein,
// Escape bricht ab, danach kehrt er auf das ausloesende Element zurueck — und die
// ABBRECHENDE Wahl ist die Vorgabe, damit die Eingabetaste niemals loescht.
//
// `wahlen` ist [{text, wert, gefahr?}]; zuletzt steht immer das Abbrechen.
function bestaetigen(text, wahlen){
  let d=document.getElementById('frage');
  if(!d||!d.showModal) return Promise.resolve(window.confirm(text)?true:false); // uralter Browser
  let vorher=document.activeElement;
  d.querySelector('#frage-text').textContent=text;
  let leiste=d.querySelector('.frage-knoepfe'); leiste.innerHTML='';
  wahlen=wahlen||[{text:t('dlg_ok'),wert:true,gefahr:1},{text:t('dlg_cancel'),wert:false}];
  return new Promise(fertig=>{
    let zu=w=>{ if(d.open)d.close(); d.onclose=null;
                if(vorher&&vorher.focus)try{vorher.focus()}catch(e){}
                fertig(w); };
    wahlen.forEach((w,i)=>{
      let b=document.createElement('button');
      b.textContent=w.text; if(w.gefahr)b.className='gefahr';
      b.onclick=()=>zu(w.wert);
      leiste.appendChild(b);
    });
    // Escape und der Klick daneben zaehlen wie die letzte Wahl — das Abbrechen.
    d.onclose=()=>zu(wahlen[wahlen.length-1].wert);
    d.showModal();
    // Fokus auf das Abbrechen, nicht auf die zerstoerende Wahl.
    let knoepfe=leiste.querySelectorAll('button');
    if(knoepfe.length)knoepfe[knoepfe.length-1].focus();
  });
}
...
async function openJobDetail(titel, plattform, el){
 ...
 // SOFORT etwas sagen, VOR dem Netzaufruf. Die Suche fragt Usenet und Archive.org und
 // brauchte gemessen 3 s bis zur ersten Rueckmeldung — bis dahin war die Zeile nur auf
 // 60 % Deckkraft, und das uebersieht man. Klicken, warten, nichts sehen, den Knopf fuer
 // kaputt halten. (#638)
 jobMeldungSetzen(jid,t('job_searching'));
 try{
  // `clean=1`: findet der rohe Release-Titel nichts, versucht der Server es gekuerzt.
  let u='/api/search?clean=1&q='+encodeURIComponent(titel)+(plattform?'&platforms='+encodeURIComponent(plattform):'');
  let a=await fetch(u);
...
function plattformMarke(slug,libSlugs){
 // ABGELEITETE PLATTFORM (#685): Sagt die Quelle nichts, der Titel liegt aber in der
 // Bibliothek, dann wissen WIR, wo — und „Plattform unbekannt" neben „in Bibliothek" waere
 // ein Widerspruch. Der Server liefert die Ordner in `lib_slugs`, aelteste Plattform zuerst
 // (das ist bei einem Titel auf mehreren Systemen fast immer die, auf der er zuerst kam).
 //
 // ABGESETZT DARGESTELLT, weil es eine Ableitung ist und keine Angabe der Quelle: der
 // erste Slug normal, die weiteren darunter gedaempft.
 if(!slug&&libSlugs&&libSlugs.length){
  let erst=SLUGNAME[libSlugs[0]]||libSlugs[0];
  let rest=libSlugs.slice(1).map(x=>SLUGNAME[x]||x);
  let h=t('plat_derived_hint').replace(/"/g,'&quot;');
  return`<span class=plat-derived title="${h}">${String(erst).replace(/</g,'&lt;')}</span>`
   +(rest.length?`<span class=plat-more>${rest.join(' · ').replace(/</g,'&lt;')}</span>`:'');}
 // Ohne Slug stand hier ein blosses `?`. Das sah nach fehlender Information aus, war aber
 // eine FOLGE: Ein Import ohne Plattform landet in `.unsortiert` (#367) — was niemand vor
 // dem Anfordern sah. Nicht geraten und nicht verworfen, sondern benannt (#621).
 if(!slug){let h=t('plat_unknown_hint').replace(/"/g,'&quot;');
  return`<span class=plat-unknown title="${h}">⚠ ${t('plat_unknown')}</span>`;}
 let name=SLUGNAME[slug]||slug;
 let sicher=String(name).replace(/</g,'&lt;');
 if(LOGOS.has(String(slug).toLowerCase()))
...
async function toggleFav(it,btn){
 ...
 // NUR den Textknoten und die Zeichenkennung tauschen. `btn.textContent=…` hat hier
 // frueher gestanden und wuerde das Zeichen mitloeschen (#708/#337).
 if(btn){btn.classList.toggle('on',!drin);
  let u=btn.querySelector('use'); if(u)u.setAttribute('href','#rs-i-'+(!drin?'herz-voll':'herz'));
  let sp=btn.querySelector('span'); if(sp)sp.textContent=t('favourite');}
...
async function loadLists(){
 ...
 let reiter=[['wish',zeichen('listen')+'<span>'+t('wishlist')+'</span>'],
            ['fav',zeichen('herz')+'<span>'+t('favourites')+'</span>']];
...
async function openProfile(){let m=document.getElementById('modal');m.style.display='block';PAV='';
 ...
 m.innerHTML=`<div class=box><button class=x onclick="closeModal()">×</button>
  <div class=sec><h3>${t('profile')} — ${(p.username||'').replace(/</g,'&lt;')}</h3>
   ...
   <div class=row><input id=pra ${inp} placeholder="${t('ra_user')}" value="${(p.ra_user||'').replace(/"/g,'&quot;')}"></div>` + ereignisKaesten('evuser', p.ereignisse, p.notify_events, t('ev_hint_user')) + `
...
async function saveProfile(){let d={display_name:document.getElementById('pdn').value,email:document.getElementById('pmail').value,lang:document.getElementById('plang').value,design:document.documentElement.dataset.design||'',webhook:document.getElementById('pwh').value,ra_user:(document.getElementById('pra')||{}).value||'',variant:readVarPrefs('p')};
 d.notify_events=ereignisLesen('evuser');
 if(PAV)d.avatar=PAV;
...
function openSettingsView(){
 ...
 // Zeichen je Reiter (#658). Die UNTEReintraege bekommen bewusst KEINE: Dort stehen
 // Produktnamen (Discord, SMTP, Telegram), und ein eigenes Zeichen dafuer zu erfinden
 // sagt weniger als der Name schon sagt. Ein Produktlogo wiederum ist nichts, was in
 // unserer Formensprache gezeichnet gehoert.
 document.getElementById('settings').innerHTML='<div class=setwrap><div class=setnav>'+
  secs.map(x=>`<a class=snav data-sec="${x[0]}" onclick="setSection('${x[0]}')">`+
    zeichen('set-'+x[0])+`<span>${x[1]}</span></a>`).join('')+
...
async function emuRollback(name){
 if(!await bestaetigen(t('emu_rb_confirm').replace('{n}',name)))return;
...
async function loadLeftovers(){
 ...
 el.innerHTML=it.map(x=>`<div class=frow style="gap:8px;align-items:center;padding:4px 0;border-bottom:1px solid var(--btn2)">
  ...
  +`<div class=frow style="margin-top:8px;gap:8px;align-items:center"><b>${mbFormat(d.total||0)}</b>
    <button class=gefahr onclick="loRemove(null)">${t('lo_removeall')}</button></div>`
  +loFehlerFeld()+loDaysFeld(d.days);}
// Was beim Import keiner Plattform zugeordnet werden konnte. Die Oberflaeche warnte an
// drei Stellen davor, dass etwas dort landen KANN, und hat den Ordner nie geoeffnet — es
// lag dort seit Tagen etwas, ohne dass es irgendwo stand. (#656)
async function loadUnsortiert(){
 let el=document.getElementById('unslist');if(!el)return;
 let d={};try{d=await(await fetch('/api/unsortiert')).json();}catch(e){el.textContent=t('st_error');return;}
 let it=d.items||[];
 ...
 el.innerHTML=`<div class=meta style="margin-bottom:6px">${t('uns_count')
  ...
  +`<div class=meta style="margin-top:8px">${t('uns_where').replace('{p}',(d.folder||'').replace(/</g,'&lt;'))}</div>`;}
...
// Die Antwort auf ein Loeschen wurde frueher weggeworfen — der Server nannte den Grund
// ("keine Schreibrechte"), die Oberflaeche zeigte davon nichts und lud nur neu. Ein
// stehengebliebener Ordner sah dann aus wie ein Klick, der nicht angekommen ist. (#645)
let loFehler=[];
async function loRemove(jid){
 if(!await bestaetigen(jid?t('lo_confirm'):t('lo_confirmall')))return;
 let d={};
 try{d=await(await fetch('/api/leftovers/remove',{method:'POST',
   headers:{'Content-Type':'application/json'},
   body:JSON.stringify(jid?{jid:jid}:{all:true})})).json();}
 catch(e){d={errors:[t('st_error')]};}
 loFehler=(d&&d.errors)||[];
 loadLeftovers();}
function loFehlerFeld(){
 if(!loFehler.length)return'';
 return `<div class=lofail><b>${t('lo_failed')}</b><ul>`
  +loFehler.map(x=>`<li>${String(x).replace(/</g,'&lt;')}</li>`).join('')+'</ul></div>';}
async function secMaint(c){
 c.innerHTML=`<h3>${t('sec_maint')}</h3><div id=mstats class=meta>…</div>
  ...
  <h3 style="margin-top:16px">${t('uns_title')}</h3>
  <div class=meta style="line-height:1.6;margin-bottom:8px">${t('uns_hint')}</div>
  <div id=unslist class=meta>…</div>
...
async function secGeneral(c){let s=await(await fetch('/api/settings')).json();let gg=s.general||{};let qo=s.quota||{};
 c.innerHTML=`<h3>${t('sec_general')}</h3>
  ...
  <div class=frow><input id=qcount type=number min=0 style="flex:0 0 90px" value="${qo.count||0}"><input id=qdays type=number min=1 style="flex:0 0 90px" value="${qo.days||7}"><span class=meta>${t('quota_count_hint')}</span></div>
  <div class=frow><input id=qbytes type=number min=0 style="flex:0 0 90px" value="${Math.round((qo.bytes||0)/1073741824)}"><span></span><span class=meta>${t('quota_bytes_hint')}</span></div>`
  + ereignisKaesten('evinst', s.ereignisse, s.notify_events, t('ev_hint_inst')) + `
...
async function regenKey(){if(!await bestaetigen('Neuen API-Key erzeugen? Alter wird ungültig. / Regenerate API key?'))return;
...
// GB im Formular, Bytes in der Einstellung (#712). Eine Grenze reicht hier von einigen
// hundert Megabyte bis in den Terabyte-Bereich; wer sie in Bytes eintippen muss, vertippt
// sich. Gerechnet wird trotzdem in Bytes, weil die Auftragsgroessen so vorliegen.
// Auswahlkaesten fuer die Ereignisse (#714). EINE Bauform fuer beide Orte: im Profil
// entscheidet sie ueber die eigenen Wege, in den Einstellungen ueber die Kanaele der
// Instanz. Zwei Kopien liefen auseinander, sobald ein Ereignis dazukommt.
//
// FEHLT ein Schluessel, ist der Kasten AN — das ist die Voreinstellung im Server, und die
// Anzeige muss sie spiegeln, sonst sieht „nicht eingestellt" nach „aus" aus.
function ereignisKaesten(id, ereignisse, auswahl, hinweis){
 return `<div class=frow style="align-items:flex-start"><span class=meta>${t('ev_title')}</span>
  <div id=${id} style="display:flex;flex-wrap:wrap;gap:12px">`
  + (ereignisse||[]).map(e=>`<label style="font-size:12px;color:var(--mut);display:flex;gap:5px;align-items:center">
      <input type=checkbox data-ev="${e}" ${(auswahl||{})[e]===false?'':'checked'}> ${t('ev_'+e)}</label>`).join('')
  + `</div></div><div class=frow><span></span><span class=meta style="font-size:11px">${hinweis}</span></div>`;}
function ereignisLesen(id){
 let raus={}; document.querySelectorAll('#'+id+' input[data-ev]').forEach(i=>{raus[i.dataset.ev]=i.checked;});
 return raus;}
async function saveQuota(){let d={quota:{enabled:document.getElementById('qen').checked,count:+document.getElementById('qcount').value,days:+document.getElementById('qdays').value,bytes:Math.round((+document.getElementById('qbytes').value||0)*1073741824)},notify_events:ereignisLesen('evinst')};
 let r=await(await fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)})).json();
...
async function orgStart(trocken){
 ...
 if(!trocken&&!await bestaetigen(t('org_confirm')))return;
...
async function deleteJob(id,hatDateien){
 let files=false;
 if(hatDateien){
  // Frueher ein `confirm`: „Abbrechen" loeschte die Anfrage TROTZDEM, nur ohne Dateien —
  // ein Abbruch war gar nicht moeglich. Jetzt drei Wahlen, und Abbrechen bricht ab. (#641)
  let w=await bestaetigen(t('del_withfiles'),[
    {text:t('del_files_yes'),wert:'beides',gefahr:1},
    {text:t('del_files_no'), wert:'nur_anfrage'},
    {text:t('dlg_cancel'),   wert:''}]);
  if(!w)return;
  files=(w==='beides');
 }else if(!await bestaetigen(t('del_confirm')))return;
...
function renderUsers(list){let ul=document.getElementById('ulist');ul.innerHTML='';
 list.forEach(u=>{let row=document.createElement('div');row.style.cssText='background:var(--hover);border-radius:8px;padding:10px;margin-bottom:8px';
  ...
  if(u.role=='admin'){let a=document.createElement('div');a.className='meta';a.style.marginTop='6px';a.textContent='alle Rechte / all permissions';row.appendChild(a);}
   ...
   row.appendChild(pg);
   // EIGENES KONTINGENT (#713). LEER heisst „globale Vorgabe", nicht „null" — deshalb
   // wird der Wert beim Leeren als leerer String geschickt und der Server entfernt den
   // Schluessel. Ohne diese Unterscheidung gaebe es keinen Weg zurueck zur globalen
   // Vorgabe, ausser den Nutzer neu anzulegen.
   let q=u.quota||{};
   let qz=document.createElement('div');
   qz.style.cssText='display:flex;flex-wrap:wrap;gap:8px;margin-top:8px;align-items:center';
   qz.innerHTML=`<span class=meta style="font-size:11px">${t('quota_own')}</span>`;
   [['count',q.count,'#'],['bytes',q.bytes==null?'':Math.round(q.bytes/1073741824),'GB']].forEach(([feld,wert,einheit])=>{
    let i=document.createElement('input');i.type='number';i.min='0';i.placeholder=einheit;
    i.value=wert==null?'':wert;
    i.style.cssText='flex:0 0 78px;padding:3px 6px;font-size:11px';
    i.onchange=()=>{
     let neu={count:qz.querySelectorAll('input')[0].value,
              bytes:qz.querySelectorAll('input')[1].value};
     let last={count:neu.count===''?'':+neu.count,
               bytes:neu.bytes===''?'':Math.round((+neu.bytes||0)*1073741824)};
     fetch('/api/users/'+encodeURIComponent(u.username),{method:'PATCH',
       headers:{'Content-Type':'application/json'},body:JSON.stringify({quota:last})});};
    qz.appendChild(i);});
   let h=document.createElement('span');h.className='meta';h.style.fontSize='11px';
   h.textContent=t('quota_own_hint');qz.appendChild(h);
   row.appendChild(qz);}
  ul.appendChild(row);});}
...
document.getElementById('q').addEventListener('input',suchKnoepfe);
window.addEventListener('popstate',suchKnoepfe);
suchKnoepfe();
