...
async function secConn(c){let vals=await(await fetch('/api/settings/connections/reveal')).json();
 ...
 c.innerHTML=`<h3>${t('sec_conn')}</h3><div class=meta style="margin-bottom:10px">${t('conn_hint')}</div>
  ...
  <input type=file id=fwfile style="display:none">
  ...
  <div id=csvc style="margin-top:10px"></div>`;raStatus();catStatus();emuStatus();fwStatus();}
...
async function fwStatus(){let el=document.getElementById('fwstat');if(!el)return;
 ...
 el.innerHTML=d.platforms.map(p=>{
   ...
   else{zeile+=fehlt.map(f=>`${f.name}: ${f.state==='fehlt'?t('fw_missing'):t('fw_badsize')}`).join(', ');}
