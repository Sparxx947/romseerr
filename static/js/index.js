...
async function secConn(c){let vals=await(await fetch('/api/settings/connections/reveal')).json();
 ...
 c.innerHTML=`<h3>${t('sec_conn')}</h3><div class=meta style="margin-bottom:10px">${t('conn_hint')}</div>
  ...
  <div id=csvc style="margin-top:10px"></div>`;raStatus();catStatus();}
...
const CONN_ALL=['sab_url','sab_apikey','sab_cat','prow_url','prow_apikey','prow_cats','igdb_id','igdb_secret','sgdb_key','ss_user','ss_pass','romm_url','romm_user','romm_pass','jd_watch','jd_out','jd_dl_base','ra_key','catalog_urls'];
