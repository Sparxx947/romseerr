...
async function openJobDetail(titel, plattform, el){
 ...
 try{
  ...
  if(!treffer){
   if(el){let m=el.querySelector('.jobmsg');
    if(m){m.textContent=t('job_no_card');setTimeout(()=>{m.textContent='';},4000);}}
   return;
  }
  window.LASTRES=d;
  openDetail(treffer);
...
function openSettingsView(){
 let secs=[['general',t('sec_general')],['notif',t('sec_notif')],['conn',t('sec_conn')],['users',t('sec_users')],['blocklist',t('blocklist')],['services',t('sec_services')],['drop',t('sec_drop')],['maint',t('sec_maint')],['tls','HTTPS'],['about',t('sec_about')]];
...
function setSection(sec,sub,ausRoute){
 ...
 ({general:secGeneral,notif:secNotif,conn:secConn,users:secUsers,blocklist:secBlocklist,services:secServices,drop:secDrop,maint:secMaint,tls:secTls,about:secAbout}[sec]||secGeneral)(c);}
