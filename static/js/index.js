...
function renderCard(it){let c=document.createElement('div');c.className='card';
 ...
 let plat=SLUGNAME[it.platform_slug]||it.platform_slug||'?';
 c.innerHTML=`<div class=cover style="${cov}"><span class=badge>${plat.replace(/</g,'&lt;')}</span><span class=src>${src}</span>`+
...
//
// ENTSCHIEDEN, wohin `denied` gehört (die offene Frage im Issue): zu **erledigt**, nicht
// zu fehlgeschlagen. Abgelehnt heißt, jemand hat entschieden — der Vorgang ist beendet und
// niemand muss etwas reparieren. Unter „fehlgeschlagen" würde man nach Defekten suchen und
// fände Entscheidungen; die Zeile selbst sagt weiterhin „Abgelehnt".
const JOBGRUPPEN={
 aktiv:   ['pending','queued','approved','downloading','importing'],
 erledigt:['done','denied'],
 fehler:  ['error']};
...
// dabei ist. `denied` zählt nicht, das war eine Entscheidung, keine offene Sache.
function jobOffen(o){return jobGruppe(o.state)!=='erledigt';}
...
async function loadAuth(){let d=await(await fetch('/api/auth/status')).json();
 ...
 cfgWarn();zeichneFuss();ladeFavs();updateJobBadge();}
